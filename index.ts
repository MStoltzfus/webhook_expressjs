...
const webhookTriggerResponse = (origin: any) => {
...
app.get("/notanymore", (req, res) => {
  res.send(
    "You did a thing! If you send a POST request to this endpoint, it should work too!"
  );
  res.status(200).end();
});
...
app.post("/cleardata", urlencodedParser, function (req, res) {
  ...
  if (req.headers["authkey"] == "1234567890") {
    console.log(
      `The submitted authkey '\ ${req.headers["authkey"]} '\ is valid`
    );
    console.log("request origin - " + origin);
    res.json({
      message:
        "This API has been deprecated during a massive refactor. Historical data is now completely handled client-side.",
    });
  } else {
    console.log(
      "POST request to " + origin + " but the authkey was not valid."
    );
